  ...
  <div class="border-b border-gray-200 pb-2">
    ...
    >
      <span class="flex items-center gap-1">
        {{ title }}
        <span v-if="activeCount > 0" class="ml-1 px-1.5 py-0.5 text-xs bg-emerald-100 text-emerald-700 rounded-full">
          {{ activeCount }}
        </span>
      </span>
      <svg
        ...
        >
          <span class="flex items-center gap-1.5">
            <span
              ...
              :class="isActive(item.value) ? 'bg-emerald-600 border-emerald-600' : 'border-gray-300'"
            >
...
const props = defineProps({
  ...
  sortBy: { type: String, default: 'count' }
})
